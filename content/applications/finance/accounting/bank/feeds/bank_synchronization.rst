...
<https://www.odoo.com/page/accounting-features>`_, and click on **See list of supported
institutions**.

More than 25,000 institutions around the world are supported.
...
You can start synchronization either by going to :menuselection:`Configuration
--> Add a Bank Account` or via the bank journal's configuration on the dashboard.
...
.. important::
   When choosing the date for the first bank statements synchronization, pick the actual date where
   you start working on your Odoo accounting database. For example, if you import your closing
   balance in Odoo on the 31.12.2022 and you start recording accounting transactions on the
...
You can find all your synchronizations by going to :menuselection:`Configuration -->
...
Or you can go to :menuselection:`Configuration --> Online Synchronization`,
select your institution and then click on the :guilabel:`fetch transactions` button.
...
:menuselection:`Configuration --> Online Synchronization`, select the connection
...
bar and type: **account_online_synchronization**. You can then click on the :guilable:`Install`
...
.. Note::
   ...
   - All previous synchronizations are disconnected during the installation and won't work anymore.
     You can find them directly in the synchronization menu (:menuselection:` Configuration -->
     Online Synchronization`). It is not possible to resynchronize these connections; you have to
     make new ones.
...
Accounting dashboard, and click on the :guilabel:`Synchronize Now` button. You can also synchronize
and fetch transactions through :menuselection:`Configuration --> Online Synchronization`.
Some providers only allow one refresh per day, so it is possible that clicking on
:guilabel:`Synchronize Now` does not get your latest transactions if you already performed such
action earlier in the day.

A transaction can be visible on your bank account, but not be fetched if it has the status
**Pending**. Only transactions with the **Posted** status will be retrieved. If it is not **Posted**
yet, you will have to wait until the status changes.
...
:menuselection:`Configuration --> Online Synchronization` to click on the
:guilabel:`Fetch Accounts` button on the connection.
...
You can update your credentials by going to :menuselection:` Configuration --> Online
Synchronization`, open the connection you want to update your credentials and click on the
